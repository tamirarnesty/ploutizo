...
describe('import service', () => {
  ...
  it('classifies a credit-side vault phrase as a settlement before persistence', async () => {
    ...
    await createImportDraft('org_1', {
      ...
      content: [
        'date,amount,description,type,category,assignee hint',
        '2026-05-02,42.18,PAYMENT THANK YOU,refund,Dining,Tamir Arnesty',
      ].join('\n'),
    });

    expect(insertImportBatchRows).toHaveBeenCalledWith(expect.anything(), [
      expect.objectContaining({
        parsedType: 'refund',
        parsedDescription: 'PAYMENT THANK YOU',
        reviewType: 'settlement',
        reviewDescription: 'Bill Payment',
        reviewCategoryId: '88888888-8888-4888-8888-888888888888',
  ...
  it('does not convert an expense vault phrase into a settlement', async () => {
    await createImportDraft('org_1', {
      ...
      fileName: 'statement.csv',
      content:
        'date,amount,description,type\n2026-05-02,42.18,PAYMENT THANK YOU,expense',
    });
  ...
  it('applies merchant-rule values and lets resolved CSV hints override them', async () => {
    vi.mocked(listCategories).mockResolvedValue([
      {
        id: '55555555-5555-4555-8555-555555555555',
        orgId: 'org_1',
    ...
    await createImportDraft('org_1', {
      ...
      fileName: 'statement.csv',
      content: [
        'date,amount,description,type,category,tags',
        '2026-05-02,42.18,STARBUCKS #12,expense,Groceries,food',
      ].join('\n'),
    });
    ...
    expect(insertImportBatchRows).toHaveBeenCalledWith(expect.anything(), [
      expect.objectContaining({
        ...
        reviewAssigneeMemberIds: ['44444444-4444-4444-8444-444444444444'],
        reviewTagIds: ['aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa'],
      }),
    ]);
  });

  it('defaults expense assignees from target-card ownership when no hint resolves', async () => {
    vi.mocked(listAccountMemberDetails).mockResolvedValue([
      {
        accountId: summaryRow.accountId,
  ...
  it('persists a valid draft when classification cannot resolve review fields', async () => {
    await createImportDraft('org_1', {
      accountId: summaryRow.accountId,
      fileName: 'statement.csv',
      content:
        'date,amount,description,type,category\n2026-05-02,42.18,Coffee,expense,Travel',
    });
  ...
  it('strips one leading spreadsheet apostrophe from a supplied external id', async () => {
    await createImportDraft('org_1', {
      accountId: summaryRow.accountId,
      fileName: 'statement.csv',
      content:
  ...
  it('leaves assignees empty when a first-name hint is ambiguous', async () => {
    vi.mocked(listOrgMembers).mockResolvedValue([
      {
        id: '44444444-4444-4444-8444-444444444444',
        orgId: 'org_1',
    ...
    await createImportDraft('org_1', {
      ...
      fileName: 'statement.csv',
      content: [
        'date,amount,description,type,assignee hint',
        '2026-05-02,42.18,Coffee,expense,Tamir',
      ].join('\n'),
    });
