/** Observability-only request headers. Never used for auth or tenancy. */
export const REQUEST_ID_HEADER = 'X-Request-Id';
export const OPERATION_ID_HEADER = 'X-Operation-Id';
export const POSTHOG_SESSION_ID_HEADER = 'X-POSTHOG-SESSION-ID';
export const POSTHOG_DISTINCT_ID_HEADER = 'X-POSTHOG-DISTINCT-ID';

/** Headers the browser must be able to read after CORS. */
export const TELEMETRY_EXPOSE_HEADERS = [
  REQUEST_ID_HEADER,
  OPERATION_ID_HEADER,
] as const;
