...
const confirmPersistIntoCollection = (
  ...
  if (!preferLive && serverRow.updatedAt >= live.updatedAt) {
    ...
    next.status = computeImportRowStatus({
      status: next.status,
      reviewType: next.reviewType,
      parsedType: next.parsedType,
      reviewCategoryId: next.reviewCategoryId,
      reviewAssigneeMemberIds: next.reviewAssigneeMemberIds,
    });
  }
  ...
  const mutate = createPacedMutations<ImportDraftRowPatchVariables>({
    onMutate: ({ patch }) => {
      ...
      collection.update(rowId, (draft) => {
        ...
        draft.status = computeImportRowStatus({
          status: draft.status,
          reviewType: draft.reviewType,
          parsedType: draft.parsedType,
          reviewCategoryId: draft.reviewCategoryId,
          reviewAssigneeMemberIds: draft.reviewAssigneeMemberIds,
        });
      });
    },
