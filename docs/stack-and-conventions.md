...
- **API:** Hono (`apps/api`)
- **Telemetry:** Vendor-neutral `@ploutizo/telemetry` contract (typed catalog + operation-scoped attributes, payload shaping, correlation IDs, local adapters). Web and API adapters implement the contract separately — see [ADR 0006](adr/0006-cross-stack-telemetry.md).
