...
- Client-side persistence in `apps/web` must use Zustand stores in `@/lib/prefs/` for localStorage prefs. SessionStorage ephemeral prefs use `@/lib/prefs/sessionPref`. Never call `localStorage`/`sessionStorage` directly in components or hooks. Exception: theme via next-themes. Key naming: `ploutizo:{feature}:{preference}`.
...
- When work matches a **GSD skill** under `.cursor/skills/` (`gsd-*`) or a **GSD workflow** under `.cursor/get-shit-done/workflows/`, **read the relevant `SKILL.md` / workflow and follow it end-to-end** — including checkpoints, subcommands, session files, and delegation steps. Do not skip orchestrator steps (for example **`/gsd-debug`** must create `.planning/debug/{slug}.md` and follow `.cursor/get-shit-done/workflows/debug.md` before improvising a fix).
