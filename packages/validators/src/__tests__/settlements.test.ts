...
describe('createSettlementSchema — assignees', () => {
  ...
  it('rejects empty assignees array', () => {
    expect(
      createSettlementSchema.safeParse({ ...validPayload, assignees: [] })
        .success
    ).toBe(false);
  });
...
describe('createSettlementSchema — pre-existing fields (regression)', () => {
  it('still rejects amountCents <= 0', () => {
    expect(
      createSettlementSchema.safeParse({ ...validPayload, amountCents: 0 })
        .success
    ).toBe(false);
    expect(
      createSettlementSchema.safeParse({ ...validPayload, amountCents: -1 })
        .success
    ).toBe(false);
  });
  ...
  it('still rejects malformed date', () => {
    expect(
      createSettlementSchema.safeParse({ ...validPayload, date: '05/08/2026' })
        .success
    ).toBe(false);
  });
});
