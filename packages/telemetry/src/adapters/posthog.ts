...
export interface PostHogLogger {
  debug: (message: string, properties?: TelemetryEmitPayload) => void;
  info: (message: string, properties?: TelemetryEmitPayload) => void;
  warn: (message: string, properties?: TelemetryEmitPayload) => void;
  error: (message: string, properties?: TelemetryEmitPayload) => void;
}
...
export interface PostHogTelemetryBridge {
  ...
  capture: (event: string, properties?: TelemetryEmitPayload) => void;
  flush?: () => Promise<void>;
}
...
export const createPostHogLevelSink = (
  ...
  const maybeCaptureWideEvent = (record: SafeTelemetryRecord) => {
    ...
    bridge.capture(record.operation, toEmitPayload(record));
  };

  return createLevelSink({
    debug: (record) => {
      bridge.logger.debug(emitMessage(record), toEmitPayload(record));
    },
    info: (record) => {
      bridge.logger.info(emitMessage(record), toEmitPayload(record));
      maybeCaptureWideEvent(record);
    },
    warn: (record) => {
      bridge.logger.warn(emitMessage(record), toEmitPayload(record));
      maybeCaptureWideEvent(record);
    },
    error: (record) => {
      bridge.logger.error(emitMessage(record), toEmitPayload(record));
      maybeCaptureWideEvent(record);
    },
  });
};
