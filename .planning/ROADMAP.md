...
- 01-05-PLAN.md — Seed scripts: seedOrgCategories + seedOrgMerchantRules + seedOrg wrapper, Clerk org.created webhook handler
...
- 02-05-PLAN.md — Gap closure: insert orgs row in webhook before seedOrg(), ReUI Combobox for tag inline-create, add missing seed icons (HeartPulse, Sparkles, MoreHorizontal)
...
Remove all per-field useState. Apply null categoryId pattern (eliminate **none** sentinel).
...
- 03.1-01-PLAN.md — Drizzle schema: transactions + transaction_assignees tables, indexes, migrations
...
- 03.2.1-01-PLAN.md — Wave 0: Install Avatar and Sonner shadcn components into packages/ui
- 03.2.1-02-PLAN.md — Wave 1: API routes (GET /, POST /invitations, DELETE /members/:memberId) + members query bug fix + data-access hooks
- 03.2.1-03-PLAN.md — Wave 2: Rewrite HouseholdSettings.tsx (3 sections) + remove Members tab + delete dead files
...
- 03.3.2-01-PLAN.md — Build Text typography primitive (complete 2026-04-17)
...
- 03.3.3-01-PLAN.md — Add error variant to text.tsx + create SwatchButton + update ColourPicker (Wave 1 foundation) (complete 2026-04-17)
- 03.3.3-02-PLAN.md — Heading migrations: replace all h1-h6 raw elements with Text across 9 files (complete 2026-04-17)
- 03.3.3-03-PLAN.md — Paragraph migrations: replace all p raw elements with Text across 16 files (complete 2026-04-17)
- 03.3.3-04-PLAN.md — Span migrations: text-content spans to Text as=span, structural spans to div, AppLogo brand mark (complete 2026-04-17)
...
- 03.4-12 through 03.4-21 — UAT-diagnosed gap closures (RefundLinker auto-fill, amount sign, ReturnType aliases, linting, data-access split, etc.)
...
- 03.4.1-08-PLAN.md — Filter operator URL persistence and API enforcement (D-25)
- 03.4.1-09-PLAN.md — Gap closure: WR-01–WR-04 filter and form fixes
- 03.4.1-10-PLAN.md — Gap closure: contribution picker, stale cache, transfer guard, date filter overhaul
- 03.4.1-11-PLAN.md — Gap closure: UAT fixes (notes preview, tag filtering, refund search, tooltip fixes)
- 03.4.1-12-PLAN.md — Gap closure: UAT re-test fixes
...
- 03.4.2-01-PLAN.md — Transaction form polish: amount recalc, notes layout, tag Enter key, category refresh
...
- 03.4.4-01-PLAN.md — Add NotFound + ErrorBoundary components and wire into createRootRoute
...
- 03.6-01-PLAN.md — Swap `@hono/clerk-auth` for `@clerk/hono`: deps, production imports/mocks/tests, `.env.example`, `stack.md` + `SUMMARY.md`, turbo typecheck
