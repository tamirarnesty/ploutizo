...
**Plans:**
1. Monorepo scaffold & package boundaries — verify pnpm workspace dep rules, JIT `validators` and `types` packages, `tsconfig` strict mode across all packages
2. Database client & migrations — initialize `postgres.js` client at module scope, create base schema, wire `db:migrate` as Railway pre-deploy command, verify transaction support
3. Clerk auth & satellite domains — configure `@hono/clerk-auth` with correct middleware order, satellite domain config on `apps/web`, `authorizedParties`, `clockSkewInMs`
4. Tenant guard & API skeleton — implement `tenantGuard()` with falsy check, establish shared response shape (`{ data }` / `{ error }`), add health endpoint
5. Seed scripts — implement `seedOrgCategories`, `seedOrgMerchantRules`, `seedOrg` wrapper; hook into org creation webhook
